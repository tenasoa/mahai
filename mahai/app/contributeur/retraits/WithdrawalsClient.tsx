...
import { ChevronLeft, ChevronRight, Wallet, History, TrendingUp, Clock, CheckCircle, AlertCircle, Smartphone, CreditCard } from 'lucide-react'
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setWithdrawAmount('')
        setPhoneNumber('')
        // Recharger la page pour afficher la nouvelle demande
        setTimeout(() => {
      ...
      <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} id="sidebar">
        ...
        <nav className="sb-nav">
          ...
          <Link className="sb-link active" href="/contributeur/retraits">
            ...
            <span className="sb-link-text">Retraits Gains</span>
          </Link>
        ...
        <div className="overlay" style={{ position: 'fixed', inset: 0, zIndex: 800, background: 'rgba(4, 3, 2, 0.76)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }} onClick={() => setShowModal(false)}>
          <div className="receipt-card" style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: 'var(--r-lg)', maxWidth: '420px', width: '100%', position: 'relative', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="receipt-body" style={{ padding: '1.1rem' }}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{ width: '100%', paddingRight: '2.5rem', padding: '0.75rem', background: 'var(--surface)', border: '1px solid var(--b1)', borderRadius: 'var(--r)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: '0.9rem' }}
                  />
              ...
              <div className="modal-cta-row" style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
                <button
                  onClick={handleWithdraw}
                  disabled={processing}
                  className="btn-gold"
                  style={{ flex: 1, fontFamily: 'var(--body)', fontSize: '0.82rem', fontWeight: 500, padding: '0.75rem', borderRadius: 'var(--r)', background: 'linear-gradient(135deg, var(--gold), var(--gold-hi))', color: 'var(--void)', border: 'none', cursor: 'none', transition: 'all 0.2s', opacity: processing ? 0.5 : 1 }}
                >
                  {processing ? 'Traitement...' : 'Confirmer'}
                </button>
                <button
                  onClick={() => setShowModal(false)}
                  style={{ flex: 1, fontFamily: 'var(--mono)', fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '0.45rem', borderRadius: 'var(--r)', background: 'transparent', border: '1px solid var(--b1)', color: 'var(--text-3)', cursor: 'none', transition: 'all 0.2s' }}
                >
                  Annuler
                </button>
              </div>
