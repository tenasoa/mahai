    ...
    <>
      <style jsx global>{`
        :root {
          ...
          --gold:#A8863A; --gold-hi:#C9A84C; --gold-lo:#D4B87A;
          --gold-dim:rgba(168,134,58,.08); --gold-line:rgba(168,134,58,.28); --gold-glow:rgba(168,134,58,.15);
        ...
        .dark {
          ...
          --gold-dim:rgba(201,168,76,.08); --gold-line:rgba(201,168,76,.22); --gold-glow:rgba(201,168,76,.18);
        ...
        .tool-icon { display: inline-block; animation: workAnim 2s ease-in-out infinite; }

        @keyframes workAnim { 0%, 100% { transform: rotate(-10deg); } 50% { transform: rotate(20deg); } }
        @keyframes glitchAnim { 0%, 90%, 100% { transform: none; opacity: 0.3; } 91% { transform: translateX(-3px); opacity: 0.6; } 92% { transform: translateX(3px); opacity: 0.4; } }
      ...
      <div className="relative min-h-screen bg-void transition-colors duration-500 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center py-20 px-8 text-center animate-[fadeUp_0.7s_ease_both]">
          ...
          <div style={{ maxWidth: '380px', margin: '0 auto 2rem', width: '100%' }}>
            ...
            <div style={{ height: '6px', background: 'var(--b2)', borderRadius: '3px', overflow: 'hidden' }}>
              <div style={{ height: '100%', background: 'linear-gradient(90deg, var(--gold), var(--amber))', borderRadius: '3px', animation: 'progressAnim 4s ease-in-out infinite' }} />
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', maxWidth: '320px', margin: '0 auto 2rem', textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.8rem', color: 'var(--text-2)' }}><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--gold)', flexShrink: 0 }} />Serveurs en ligne · Trafic redirigé</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.8rem', color: 'var(--text-2)' }}><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--gold)', flexShrink: 0 }} />Sauvegardes complètes · Données sécurisées</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.8rem', color: 'var(--text-2)' }}><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--amber)', animation: 'blink 1.5s ease-in-out infinite', flexShrink: 0 }} />Migration base de données en cours</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.8rem', color: 'var(--text-2)' }}><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--b2)', border: '1px solid var(--b1)', flexShrink: 0 }} />Déploiement · En attente</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.8rem', color: 'var(--text-2)' }}><span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--b2)', border: '1px solid var(--b1)', flexShrink: 0 }} />Tests de validation · En attente</div>
          </div>
          ...
          <div style={{ maxWidth: '340px', margin: '0 auto', display: 'flex', gap: '0.5rem' }}>
            <input
              ...
              style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--b2)', borderRadius: '4px', padding: '0.55rem 0.85rem', fontFamily: 'var(--body)', fontSize: '0.82rem', color: 'var(--text)', outline: 'none' }}
              value={notifyEmail}
              onChange={(e) => setNotifyEmail(e.target.value)}
            />
            <button
              onClick={handleNotify}
              style={{ fontFamily: 'var(--body)', fontSize: '0.8rem', fontWeight: 500, padding: '0.55rem 1rem', borderRadius: '4px', background: 'linear-gradient(135deg, var(--gold), var(--gold-hi))', color: 'var(--void)', border: 'none', whiteSpace: 'nowrap', transition: 'all 0.2s' }}
            >{isSubscribed ? '✓ Inscrit !' : 'Me notifier'}</button>
          </div>
        ...
        <div style={{ position: 'relative', bottom: '1.5rem', marginTop: '2rem', zIndex: 20, display: 'flex', gap: '0.4rem', background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: '8px', padding: '0.35rem', backdropFilter: 'blur(12px)', width: 'fit-content', marginLeft: 'auto', marginRight: 'auto' }}>
          {(['maintenance', 'overload', 'scheduled'] as VariantKey[]).map(key => (
            <button
              key={key}
              onClick={() => setCurrentVariant(key)}
              style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '0.28rem 0.75rem', borderRadius: '4px', border: '1px solid transparent', background: currentVariant === key ? 'var(--amber)' : 'transparent', color: currentVariant === key ? 'var(--void)' : 'var(--text-3)', transition: 'all 0.2s' }}
            >{key === 'maintenance' ? '503 Main.' : key === 'overload' ? '503 Sur.' : 'Planifiée'}</button>
          ))}
        </div>
      </div>
    </>
