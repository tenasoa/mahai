...
export function RichTextEditor({
  ...
  return (
    <div
      className={`border rounded-xl bg-card transition-colors ${isFocused ? "border-gold shadow-glow-sm" : "border-border-1"}`}
    >
      {/* Toolbar */}
      <div
        role="toolbar"
        aria-label="Barre d'outils de formatage"
        className="flex flex-wrap items-center gap-1 p-2 border-b border-border-1 bg-surface sticky top-[-24px] z-20 shadow-md transition-shadow rounded-t-xl"
      >
        <ToolbarButton
          onClick={() => formatBlock("<P>")}
          icon={Type}
          title="Texte normal"
          active={document.queryCommandValue("formatBlock") === "p"}
        />
        ...
        <div className="w-[1px] h-6 bg-border-1 mx-1" />
        <ToolbarButton
          onClick={() => setLinkModal({ ...linkModal, open: true })}
          icon={LinkIcon}
          title="Insérer un lien"
        />
        <ToolbarButton
          onClick={() => setImageModal({ ...imageModal, open: true })}
          icon={ImageIcon}
          ...
          <div className="bg-card border border-gold-line rounded-xl shadow-lg w-full max-w-sm p-5 animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-text-4 hover:text-ruby"
                aria-label="Fermer"
              >
                <X size={20} />
              </button>
            ...
            <div className="space-y-4">
              <div>
                <label
                  htmlFor={linkUrlId}
                  className="text-xs font-mono text-text-4 uppercase mb-1 block"
                >
                  URL du lien
                </label>
                <input
            ...
            <div className="space-y-4">
              ...
              >
                {isUploading ? (
                  ...
                  <>
                    <Upload
                      className="mx-auto mb-2 text-text-4 group-hover:text-gold"
                      size={24}
                    />
                    <p className="text-sm text-text-3">
                      Cliquez pour uploader depuis votre ordinateur
                    </p>
                  </>
                )}
                <input
              ...
              <div className="relative text-center py-2">
                <span className="bg-card px-2 text-[10px] font-mono text-text-4 uppercase relative z-10">
                  Ou via URL
                </span>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setImageModal({ ...imageModal, url: e.target.value })
                  }
                />
                <button
