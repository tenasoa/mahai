...
> The parent `../AGENTS.md` covers the wider monorepo (BMAD workflows); this file is scoped to the `mahai/` Next.js app.
...
There is **no `prisma` script** — the app moved off Prisma to raw SQL via `pg`. Ignore references to `pnpm prisma …` in older docs (including the top-level `../AGENTS.md` and `DATABASE_MIGRATION.md`). Schema changes are applied via the SQL files in `migrations_manual/` and `supabase/migrations/`, run from the Supabase SQL Editor.
