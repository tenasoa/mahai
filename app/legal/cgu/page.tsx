...
export default function CGUPage() {
  ...
  return (
    <div className="min-h-screen bg-void text-text cursor-none">
      {/* Custom cursor */}
      <div 
        className="fixed w-2 h-2 bg-gold rounded-full z-50 pointer-events-none mix-blend-screen transition-all duration-200"
        style={{
          left: mousePos.x - 4,
          top: mousePos.y - 4,
        }}
      />
      <div 
        className="fixed w-9 h-9 border border-gold/40 rounded-full z-40 pointer-events-none transition-all duration-300"
        style={{
          left: cursorPos.x - 18,
          top: cursorPos.y - 18,
        }}
      ...
      {/* Navigation */}
      <nav className="sticky top-0 z-50 bg-void/94 border-b border-b1 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
          <Link href="/" className="flex items-center gap-1 text-display font-semibold text-lg text-text">
            Mah<span className="w-2 h-2 bg-gold rounded-full shadow-gold-sm animate-pulse"></span>AI
          </Link>
          <Link
            href="/auth/register"
            className="font-mono text-xs text-text3 hover:text-text2 transition-colors"
          >
      ...
      {/* Hero */}
      <div className="border-b border-b1 bg-depth relative z-10 py-14">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-wrap gap-5 font-mono text-xs text-text3">
      ...
      {/* Tabs */}
      <div className="border-b border-b1 bg-depth sticky top-16 z-40">
      ...
      {/* Layout */}
      <div className="max-w-7xl mx-auto px-6 py-12 pb-24 grid grid-cols-[220px_1fr] gap-12 relative z-10">
        {/* TOC */}
        <aside className="sticky top-32 max-h-[calc(100vh-160px)] overflow-y-auto scrollbar-hide">
          <div className="font-mono text-[0.57rem] text-text4 uppercase tracking-[0.16em] mb-3">
            Sommaire
          </div>
          {tocItems.map((item) => (
            <a
              ...
              className={`block font-mono text-[0.64rem] py-2 px-0 pl-3 border-l-2 transition-all cursor-pointer ${
                ...
                activeSection === item.id
                  ? 'text-gold border-gold'
                  : 'text-text3 border-b2 hover:text-gold hover:border-gold'
              }`}
            >
              {item.label}
            </a>
        ...
        <div className="doc-content">
          ...
          <h2 className="font-display font-normal text-4xl text-text tracking-[-0.03em] mt-12 mb-4 pt-8 border-t border-b2 first:mt-0 first:pt-0 first:border-t-0 leading-[1.1]">
          ...
          <div className="bg-card border border-gold-line rounded-lg p-5 my-5 flex gap-3 items-start">
            <Info className="w-5 h-5 text-gold flex-shrink-0 mt-1" />
            <div className="text-[0.84rem] text-text2 leading-relaxed">
          ...
          <ul className="pl-6 mb-4">
            <li className="text-text2 leading-relaxed mb-1"><strong>Plateforme</strong> : le site web et l'application mobile Mah.AI ;</li>
          ...
          <table className="w-full border-collapse my-4">
            ...
            <tbody>
              {creditPacks.map((pack, index) => (
                <tr key={index} className="hover:bg-b3 transition-colors">
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{pack.pack}</td>
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{pack.credits}</td>
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{pack.price}</td>
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{pack.validity}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <ul className="pl-6 mb-4">
            <li className="text-text2 leading-relaxed mb-1">Être l'auteur ou disposer des droits nécessaires à la diffusion du contenu ;</li>
            <li className="text-text2 leading-relaxed mb-1">Que le contenu est exact, fidèle au programme officiel et ne viole aucune règle de propriété intellectuelle ;</li>
            <li className="text-text2 leading-relaxed mb-1">Que le contenu ne contient aucun élément illicite, discriminatoire ou diffamatoire.</li>
          </ul>
          <p className="text-text2 leading-relaxed mb-4">
          ...
          <table className="w-full border-collapse my-4">
            <thead>
              <tr>
                <th className="bg-lift font-mono text-[0.6rem] uppercase tracking-[0.1em] text-text3 py-3 px-4 text-left border-b border-b1">Statut contributeur</th>
                <th className="bg-lift font-mono text-[0.6rem] uppercase tracking-[0.1em] text-text3 py-3 px-4 text-left border-b border-b1">Commission Mah.AI</th>
                <th className="bg-lift font-mono text-[0.6rem] uppercase tracking-[0.1em] text-text3 py-3 px-4 text-left border-b border-b1">Revenu contributeur</th>
              </tr>
            </thead>
            <tbody>
              {commissionTiers.map((tier, index) => (
                <tr key={index} className="hover:bg-b3 transition-colors">
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{tier.status}</td>
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{tier.commission}</td>
                  <td className="py-3 px-4 text-[0.84rem] text-text2 border-b border-b3">{tier.revenue}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="bg-ruby/8 border border-ruby/25 rounded-lg p-4 my-5">
          ...
          {/* Footer info */}
          <div className="mt-12 p-6 bg-card border border-b1 rounded-lg text-center">
            <div className="font-mono text-[0.58rem] text-text3 uppercase tracking-[0.14em] mb-3">Dernière mise à jour</div>
            <div className="font-display text-lg text-text2">1er janvier 2026 · Version 2.0</div>
            <div className="font-mono text-[0.62rem] text-text4 mt-2">Mah.AI SAS · Antananarivo, Madagascar</div>
          </div>
        </div>
      </div>

      {/* Sticky Accept Footer */}
      <div className="fixed bottom-0 left-0 right-0 bg-void/96 border-t border-b1 p-4 flex items-center justify-center gap-4 z-50 backdrop-blur-lg">
        <div className="font-mono text-xs text-text4 max-w-lg text-center">
          En vous inscrivant sur Mah.AI, vous acceptez les présentes CGU et notre politique de confidentialité.
        </div>
        <button
          onClick={handleAccept}
          className="font-body text-sm font-medium px-8 py-3 rounded bg-gradient-to-r from-gold to-gold-hi text-void border-none transition-all hover:-translate-y-0.5 hover:shadow-gold-md tracking-[0.04em]"
        >
