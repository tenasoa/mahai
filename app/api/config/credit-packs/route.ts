...
export async function GET() {
  try {
    ...
    const arPerCredit = configResult.rows[0]?.arPerCredit || 50
    ...
    const packs = result.rows.map(row => ({
      ...
      name: row.name,
      credits: row.credits,
      price: row.credits * arPerCredit, // Dynamically computed from global arPerCredit
      bonus: row.bonus || 0,
      popular: row.isPopular || false
    }))

    return NextResponse.json({ packs, arPerCredit })
  } catch (err) {
