...
import { Sparkles, Zap, Brain, Target, Languages, Clock, Shield, CheckCircle, ArrowRight, ArrowLeft } from 'lucide-react'
    ...
    <div className="min-h-screen bg-void text-text">
      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-text2 text-center max-w-2xl mx-auto mb-12">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {advantages.map((advantage, index) => (
              <div key={index} className="bg-card border border-b1 rounded-lg overflow-hidden">
                <div className="bg-lift border-b border-b1 p-4">
                  <h3 className="font-display text-xl text-text">{advantage.title}</h3>
                </div>
                <div className="p-6">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-b3">
                        <th className="font-mono text-xs text-text3 uppercase tracking-[0.1em] text-left py-3">Critère</th>
                        <th className="font-mono text-xs text-gold uppercase tracking-[0.1em] text-left py-3">Mah.AI</th>
                        <th className="font-mono text-xs text-text3 uppercase tracking-[0.1em] text-left py-3">Autres</th>
                      </tr>
                    </thead>
                    <tbody>
                      {advantage.items.map((item, i) => (
                        <tr key={i} className="border-b border-b3 last:border-b-0">
                          <td className="py-3 text-sm text-text2">{item.label}</td>
                          <td className="py-3 text-sm text-gold font-medium">{item.ours}</td>
                          <td className="py-3 text-sm text-text3">{item.theirs}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            <div className="bg-card border border-b1 rounded-lg p-8 flex gap-6">
              <div className="flex-shrink-0 w-12 h-12 bg-gold-dim border border-gold-line rounded flex items-center justify-center text-gold font-display text-2xl">
                1
              </div>
              <div>
