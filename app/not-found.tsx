...
export default function NotFound() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      for (let i = 0; i < 90; i++) pts.push({ x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - .5) * .3, vy: (Math.random() - .5) * .3, r: Math.random() * 1.5 + .5, a: Math.random() })
    }
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        const alpha = d < 200 ? p.a * (1 - d / 200) * .6 : p.a * .12
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fillStyle = `rgba(${gold}, ${alpha})`; ctx.fill()
      })
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          const d = Math.hypot(pts[i].x - pts[j].x, pts[i].y - pts[j].y)
          if (d < 90) { ctx.beginPath(); ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y); ctx.strokeStyle = `rgba(${gold}, ${(1 - d / 90) * .04})`; ctx.lineWidth = .5; ctx.stroke() }
        }
      }
      ...
      <style jsx global>{`
        ...
        * { cursor: none !important; outline: none !important; border: none !important; box-shadow: none !important; }
        ...
        .sand-icon {
          ...
          animation: flipSand 4s ease-in-out infinite;
        }
        ...
        @keyframes gp { 0%, 100% { box-shadow: 0 0 8px var(--gold-glow); } 50% { box-shadow: 0 0 20px rgba(201,168,76,0.4); } }
      `}</style>

      <div className="fixed inset-0 bg-void transition-colors duration-500">
        ...
        <nav className="fixed top-0 left-0 right-0 z-[300] bg-void/85 border-b border-b1 backdrop-blur-xl">
        ...
        <div className="fixed inset-0 z-[2] flex flex-col items-center justify-center p-8 text-center">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 0, maxWidth: '440px', width: '100%', margin: '0 auto' }}>
            <input type="text" placeholder="Rechercher un sujet, matière, niveau…" style={{ flex: 1, background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: '4px 0 0 4px', padding: '0.7rem 1rem', fontFamily: 'var(--body)', fontSize: '0.85rem', color: 'var(--text)', outline: 'none' }} value={searchQuery} onChange={(e) => setSearchInput(e.target.value)} />
            <button style={{ background: 'var(--gold)', color: '#fff', border: 'none', padding: '0.7rem 1.1rem', borderRadius: '0 4px 4px 0', fontFamily: 'var(--body)', fontSize: '0.82rem', fontWeight: 500, whiteSpace: 'nowrap' }}>Rechercher →</button>
          </div>
        </div>

        <div style={{ position: 'fixed', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '0.5rem', zIndex: 10 }}>
          {(['404', '403', '500', 'session'] as VariantKey[]).map(k => (
            <button key={k} onClick={() => setCurrentVariant(k)} style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '0.32rem 0.75rem', borderRadius: '2px', border: '1px solid var(--b1)', background: currentVariant === k ? 'var(--gold-dim)' : 'var(--surface)', color: currentVariant === k ? 'var(--gold)' : 'var(--text-3)', transition: 'all 0.2s' }}>
              {k}
            </button>
          ))}
        </div>
