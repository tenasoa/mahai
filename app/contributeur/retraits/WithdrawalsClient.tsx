    ...
    <div className="admin-page-content">
      ...
      <div className="kpi-grid">
        <div className="kpi-card">
          ...
          <div className="kpi-value gold">{balance.available.toLocaleString('fr-FR')} <small>Ar</small></div>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value">{balance.pending.toLocaleString('fr-FR')} <small>Ar</small></div>
          <div className="kpi-trend" style={{ color: 'var(--amber)' }}>En cours de traitement</div>
        </div>

        <div className="admin-card">
          <div className="admin-card-body">
            <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1.5rem' }}>Détails de vos gains</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'var(--surface)', borderRadius: 'var(--r)', border: '1px solid var(--b2)' }}>
                <span style={{ color: 'var(--text-3)' }}>Revenu Brut Total</span>
                <span style={{ fontWeight: 600 }}>{stats.totalEarnings.toLocaleString('fr-FR')} Ar</span>
              </div>
              <div style={{ borderTop: '1px dashed var(--b2)', margin: '0.5rem 0' }}></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'var(--gold-dim)', borderRadius: 'var(--r)', border: '1px solid var(--gold-line)' }}>
                <span style={{ color: 'var(--gold)', fontWeight: 600 }}>Solde Net Disponible</span>
                <span style={{ color: 'var(--gold)', fontWeight: 700, fontSize: '1.1rem' }}>{stats.balance.toLocaleString('fr-FR')} Ar</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value">{stats.thisMonth.toLocaleString('fr-FR')} <small>Ar</small></div>
          <div className="kpi-trend" style={{ color: 'var(--blue)' }}>Moyenne: {stats.averageWithdrawal.toLocaleString('fr-FR')} Ar</div>
        </div>
      </div>

      <div className="cycle-banner" style={{ margin: '2.5rem 0', padding: '1.5rem 2.5rem' }}>
        <div className="cb-item">
          <span className="cb-label">Cycles de paiement</span>
          <span className="cb-val" style={{ color: 'var(--gold)' }}>Tous les Mardis & Vendredis</span>
        </div>
      ...
      <div className="admin-tabs">
        <button className={`admin-tab ${activeTab === 'all' ? 'admin-tab-active' : ''}`} onClick={() => setActiveTab('all')}>Historique complet</button>
        <button className={`admin-tab ${activeTab === 'pending' ? 'admin-tab-active' : ''}`} onClick={() => setActiveTab('pending')}>En attente</button>
        <button className={`admin-tab ${activeTab === 'completed' ? 'admin-tab-active' : ''}`} onClick={() => setActiveTab('completed')}>Versements effectués</button>
        <button className={`admin-tab ${activeTab === 'failed' ? 'admin-tab-active' : ''}`} onClick={() => setActiveTab('failed')}>Échecs</button>
      </div>

      <div className="admin-card">
        <div className="admin-card-body" style={{ paddingBottom: '0.5rem' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 600 }}>Historique des paiements</h3>
        </div>
            ...
            <tbody>
              {filteredWithdrawals.length === 0 ? (
                <tr>
                  <td colSpan={5}>
                    <div className="admin-empty-state">
                      ...
                      <div className="admin-empty-state-text">Aucun retrait trouvé dans cette catégorie</div>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredWithdrawals.map((w) => {
                  ...
                  if (w.status === 'PENDING' || w.status === 'PROCESSING') badgeClass = 'status-amber'
                  ...
                  return (
                    <tr key={w.id}>
                      ...
                      <td>
                        <span style={{ fontFamily: 'var(--mono)', fontSize: '0.9rem', color: 'var(--text-2)' }}>{w.amount.toLocaleString('fr-FR')} Ar</span>
                      </td>
                      <td>
                        <span style={{ fontFamily: 'var(--mono)', fontSize: '0.85rem', color: 'var(--text-4)' }}>-{fee.toLocaleString('fr-FR')} Ar</span>
                      </td>
                      <td>
      ...
      {showModal && (
        <div className="receipt-overlay" onClick={() => setShowModal(false)}>
          <div className="receipt-card" style={{ maxWidth: '450px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="receipt-body">
                  ...
                  {MOBILE_MONEY_OPERATORS.map((op) => (
                    <button
                      ...
                      style={{ 
                        ...
                        height: 'auto',
                        borderColor: selectedOperator === op.id ? op.color : 'var(--b2)',
                        background: selectedOperator === op.id ? op.color + '15' : 'transparent',
                        color: selectedOperator === op.id ? op.color : 'var(--text-3)'
                      }}
                    >
                      <Smartphone size={16} style={{ marginBottom: '4px' }} />
                      <span style={{ fontSize: '0.65rem', fontWeight: 600 }}>{op.name}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <NumberInput 
                  ...
                  disabled={processing} 
                />
                <p style={{ fontSize: '0.75rem', color: 'var(--text-4)', marginTop: '0.5rem', fontFamily: 'var(--mono)' }}>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ position: 'relative', marginTop: '0.5rem' }}>
                  <input 
                    ...
                    style={{ paddingRight: '3rem', fontSize: '1.1rem', letterSpacing: '0.05em' }}
                  />
                  <div style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-4)' }}>
                    <Smartphone size={20} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ 
                ...
                <div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-3)', textTransform: 'uppercase' }}>Solde disponible</div>
                  <div style={{ fontSize: '1.25rem', color: 'var(--gold)', fontWeight: 600 }}>{balance.available.toLocaleString('fr-FR')} Ar</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-3)', textTransform: 'uppercase' }}>Net reçu</div>
                  <div style={{ fontSize: '1.25rem', color: 'var(--text)', fontWeight: 600 }}>
                    {(parseInt(withdrawAmount || '0') * 0.99).toLocaleString('fr-FR')} Ar
                  </div>
                </div>
              </div>
            </div>

            <div className="receipt-footer" style={{ borderTop: 'none', paddingTop: 0 }}>
              <div style={{ display: 'flex', gap: '0.75rem', width: '100%' }}>
                ...
                <button 
                  ...
                  style={{ flex: 2 }}
                >
                  {processing ? <Loader2 size={18} className="animate-spin" /> : 'Confirmer le retrait'}
                </button>
              </div>
            </div>
          </div>
