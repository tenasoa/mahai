...
export default function AnalyticsClient({
  ...
  const maxEarnings = Math.max(
    ...earningsHistory.map((h: any) => Number(h.earnings) || 0),
    1,
  );
  const gridLabels = [
    maxEarnings,
    Math.round(maxEarnings * 0.75),
    Math.round(maxEarnings * 0.5),
    Math.round(maxEarnings * 0.25),
    0,
  ];
  ...
  return (
    <div className="admin-page-content">
      ...
      {/* KPI Stats Grid */}
      <div
        className="kpi-grid contrib-animate-in"
        style={{
      ...
      >
        <div className="kpi-card">
          ...
          <div className="kpi-value gold">
            {Number(stats.totalEarnings).toLocaleString("fr-FR")}{" "}
            <small>Ar</small>
          </div>
          <div className="kpi-trend">
            ...
            {Number(stats.totalSales) > 0
              ? Math.round(
                  Number(stats.totalEarnings) / Number(stats.totalSales),
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value">
            {Number(stats.totalSales)} <small>ventes</small>
          </div>
        ...
        <div className="kpi-card">
          ...
          >
            {Number(stats.averageRating) >= 4.5
              ? "Excellent"
              : Number(stats.averageRating) >= 3.5
                ? "Bien"
                : "N/A"}
          </div>
        </div>
        ...
        <div className="admin-card">
          <div className="admin-card-body">
            <div className="contrib-card-header">
              ...
              <div
                className="contrib-period-switcher"
                role="tablist"
                aria-label="Période"
              >
                {periods.map((p) => (
                  ...
                  return (
                    <div key={index} className="contrib-chart-col">
                      ...
                      <div className="contrib-chart-tooltip">
                        {earnings.toLocaleString("fr-FR")} Ar
                        <br />
              ...
              {topSubjects.length === 0 ? (
                ...
                topSubjects.map((subject: any, index: number) => (
                  <div key={subject.id} className="contrib-top-row">
                    <div
                      className={`contrib-top-rank ${index === 0 ? "is-first" : ""}`}
                    >
                      {index + 1}
                    </div>
                    <div className="contrib-top-body">
                      <div className="contrib-top-title">{subject.titre}</div>
                      <div className="contrib-top-meta">
                        {Number(subject.sales) || 0} vente
                        {Number(subject.sales) > 1 ? "s" : ""}
                      </div>
                    </div>
                    <div className="contrib-top-val">
                      {(Number(subject.revenue) || 0).toLocaleString("fr-FR")}{" "}
                      Ar
                    </div>
                  </div>
                ))
                  ...
                  <tr key={subject.id}>
                    <td>
                      <div className="contrib-subject-cell">
                        <span className="contrib-subject-title">
                          {subject.titre}
                        </span>
                        ...
                        {Number(subject.rating) > 0 ? (
                          <>
                            ...
                            <span style={{ fontWeight: 600 }}>
                              {Number(subject.rating).toFixed(1)}
                            </span>
                          </>
                        ) : (
        ...
        {sortedStats.length > PAGE_SIZE && (
          <div className="contrib-pagination">
            <span className="contrib-pagination-info">
              ...
              {Math.min(currentPage * PAGE_SIZE, sortedStats.length)} sur{" "}
              {sortedStats.length}
            </span>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, idx, arr) => (
                  ...
                  >
                    {idx > 0 && arr[idx - 1] !== p - 1 && (
                      <span
                        style={{ color: "var(--text-4)", padding: "0 4px" }}
                      >
                        …
                      </span>
                    )}
